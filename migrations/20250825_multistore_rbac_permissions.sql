-- DebitManager: permissions SQL alignées sur le modèle Magasinier / Gérant validé.
begin;
...
begin
  ...
  base_allowed := case p_permission
    ...
    when 'stock.receive' then employee_position in ('MAGASINIER', 'APPROVISIONNEMENT', 'ADMINISTRATEUR')
    when 'stock.issue' then employee_position in ('APPROVISIONNEMENT', 'ADMINISTRATEUR')
    when 'stock.adjust' then employee_position in ('MAGASINIER', 'GERANT', 'APPROVISIONNEMENT', 'ADMINISTRATEUR')
    when 'stock.handoff' then employee_position in ('MAGASINIER', 'ADMINISTRATEUR')
    when 'stock.accept_counter' then employee_position in ('GERANT', 'ADMINISTRATEUR')
    when 'products.manage' then employee_position in ('MAGASINIER', 'GERANT', 'ADMINISTRATEUR')
