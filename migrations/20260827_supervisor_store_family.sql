-- DebitManager: séparation métier des magasins principaux pour le catalogue et les achats du Superviseur.
begin;
...
alter table public.inventory_stores
  drop constraint if exists inventory_stores_stock_family_check;

alter table public.inventory_stores
  add constraint inventory_stores_stock_family_check
  check (stock_family in ('BEVERAGE', 'KITCHEN'));

-- Les magasins historiques sont conservés. Les noms explicitement liés à la cuisine sont classés cuisine ; le reste reste boissons par compatibilité.
update public.inventory_stores
set stock_family = 'KITCHEN', updated_at = now()
where is_active
  and stock_family = 'BEVERAGE'
  and (lower(name) like '%cuisine%' or lower(name) like '%kitchen%');
...
begin
  ...
  if store_row.stock_family <> product_row.stock_family then raise exception 'STORE_PRODUCT_FAMILY_MISMATCH'; end if;
