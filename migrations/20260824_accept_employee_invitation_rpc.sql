-- DebitManager only: server-side invitation acceptance. The raw token is never persisted.
...
begin
  ...
  values (v_user_id, v_invitation.tenant_id, v_invitation.first_name, v_invitation.last_name, lower(v_invitation.email), 'TENANT_STAFF', v_invitation.position, 'ACTIVE')
  ...
  where tenant_id = v_invitation.tenant_id and user_id = v_user_id and deleted_at is null
  limit 1;

  if v_employee_id is null then
    insert into public.employees (tenant_id, user_id, first_name, last_name, position, status)
    values (v_invitation.tenant_id, v_user_id, v_invitation.first_name, v_invitation.last_name, v_invitation.position, 'ACTIVE')
  ...
  set status = 'ACCEPTED', accepted_at = now(), accepted_user_id = v_user_id, employee_id = v_employee_id
