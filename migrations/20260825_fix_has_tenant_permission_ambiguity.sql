-- DebitManager: fix ambiguous employee_id reference in the tenant permission function.
-- This preserves the existing permission matrix and only removes the PL/pgSQL name collision.
...
begin
  ...
  v_base_allowed := case p_permission
    ...
    when 'orders.create' then v_employee_position in ('SERVEUR', 'SUPERVISEUR', 'GERANT', 'BARMAN', 'ADMINISTRATEUR')
    ...
    when 'payments.create' then v_employee_position in ('SERVEUR', 'SUPERVISEUR', 'GERANT', 'BARMAN', 'ADMINISTRATEUR')
