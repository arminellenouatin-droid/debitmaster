...
export function TablesClient({ canManage }: { canManage: boolean }) {
  ...
  type PosterTarget = { label: string; zone: string | null; public_menu_url: string | null; kind: "table" | "room" };
  const [poster, setPoster] = useState<{ target: PosterTarget; qrDataUrl: string } | null>(null);
  ...
  async function openQrPoster(target: PosterTarget) {
    setError("");
    setMessage("");
    try {
      if (!target.public_menu_url) throw new Error("QR_NOT_CONFIGURED");
      const qrDataUrl = await QRCode.toDataURL(target.public_menu_url, { width: 900, margin: 3, errorCorrectionLevel: "H", color: { dark: "#000000", light: "#ffffff" } });
      setPoster({ target, qrDataUrl });
    } catch (cause) {
      setError(cause instanceof Error && cause.message === "QR_NOT_CONFIGURED" ? `${target.kind === "room" ? "Chambre" : "Table"} affichée, mais le QR nécessite la configuration du secret serveur.` : `Impossible de générer l’affiche QR de cette ${target.kind === "room" ? "chambre" : "table"}.`);
    }
  }
  ...
  function printPoster() {
    ...
    const targetLabel = poster.target.kind === "room" ? `CHAMBRE ${poster.target.label}` : `TABLE ${poster.target.label}`;
    const targetContext = poster.target.zone || (poster.target.kind === "room" ? "Auberge" : "Zone non renseignée");
  ...
  async function downloadPoster() {
    ...
    const qrImage = new Image(); qrImage.onload = () => { context.fillStyle = "#fff"; context.fillRect(260, 760, 1280, 1280); context.drawImage(qrImage, 330, 830, 1140, 1140); context.fillStyle = "#fff"; context.font = "bold 48px Arial"; context.fillText(`${targetLabel} · ${targetContext}`, 900, 2170); context.fillStyle = "#d9a83e"; context.font = "italic 120px Georgia"; context.fillText("Merci", 900, 2350); context.fillStyle = "#fff"; context.font = "bold 42px Arial"; context.fillText(`☎ ${info.phone}`.slice(0, 34), 900, 2460); context.font = "34px Arial"; context.fillText(info.address.slice(0, 52), 900, 2530); canvas.toBlob((blob) => { if (!blob) return; const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `affiche-qr-${targetLabel.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.png`; link.click(); URL.revokeObjectURL(link.href); }, "image/png"); }; qrImage.src = poster.qrDataUrl;
  }
              ...
              {tables.map((table) => (
                <article key={table.id} className={`rounded-xl border-2 p-5 ${statusStyles[table.status]}`}>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-3">
                    ...
                    {canManage && <button type="button" onClick={() => void openQrPoster({ label: table.label, zone: table.zone, public_menu_url: table.public_menu_url, kind: "table" })} className="inline-flex text-xs font-black text-[var(--primary)] underline underline-offset-4">Télécharger le QR</button>}
