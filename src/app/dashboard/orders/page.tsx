...
export default async function OrdersPage({ searchParams }: { searchParams: Promise<{ table?: string }> }) {
  ...
  const active = await getActiveTenantContext();
  if (authorization.role === "ADMINISTRATEUR" || (authorization.role === "SUPERVISEUR" && active.company?.activity_type === "POWER")) return <DashboardShell firstName={auth.user.user_metadata?.first_name ?? "propriétaire"}><OwnerOrdersClient tenantId={active.tenantId ?? ""} companyName={active.company?.name ?? "Établissement actif"} /></DashboardShell>;
