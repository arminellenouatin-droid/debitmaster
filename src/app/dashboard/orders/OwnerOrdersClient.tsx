...
export function OwnerOrdersClient({ tenantId, companyName }: { tenantId: string; companyName: string }) {
  ...
  return <div className="space-y-7">
    ...
    <section className="rounded-2xl bg-[var(--surface)] p-6 shadow-[0_10px_35px_rgba(45,38,29,0.06)]"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-[var(--secondary)]">File opérationnelle</p><h2 className="mt-2 text-xl font-black text-[var(--primary)]">Toutes les commandes</h2></div><div className="flex flex-wrap gap-2"><select value={range} onChange={(event) => setRange(event.target.value)} className="rounded-lg border border-[var(--line)] bg-[var(--surface)] px-3 py-2 text-sm font-bold text-[var(--primary)]"><option value="today">Aujourd’hui</option><option value="7d">7 derniers jours</option><option value="30d">30 derniers jours</option><option value="90d">90 derniers jours</option></select><select value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-lg border border-[var(--line)] bg-[var(--surface)] px-3 py-2 text-sm font-bold text-[var(--primary)]"><option value="">Tous les états</option>{Object.entries(statusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><select value={server} onChange={(event) => setServer(event.target.value)} className="rounded-lg border border-[var(--line)] bg-[var(--surface)] px-3 py-2 text-sm font-bold text-[var(--primary)]"><option value="">Toutes les personnes</option>{(data?.serveuses ?? []).map((item) => <option key={item.user_id} value={item.user_id}>{item.first_name} {item.last_name}</option>)}</select><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="N° commande, serveur, table" className="rounded-lg border border-[var(--line)] bg-[var(--surface)] px-3 py-2 text-sm outline-none focus:border-[var(--primary)]" /></div></div>
      <div className="mt-6 grid gap-3 md:grid-cols-2">{orders.map((order) => { const kinds = orderKinds(order); const expanded = expandedOrderId === order.id; return <article key={order.id} className="rounded-xl border border-[var(--line)] p-4 transition hover:border-[var(--primary)]"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="font-black text-[var(--primary)]">{order.order_number} <span className="font-medium text-[var(--muted)]">· {order.table_label || "Comptoir"}</span></p><p className="mt-1 text-xs text-[var(--muted)]">{order.server_name || "Personne non renseignée"} · {new Date(order.created_at).toLocaleString("fr-FR")}</p></div><span className={`w-fit rounded-full px-3 py-1.5 text-xs font-black ${statusTone[order.status] ?? "bg-[var(--surface-muted)] text-[var(--primary)]"}`}>{statusLabels[order.status] ?? order.status}</span></div><div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-bold"><span className="rounded-full bg-[var(--surface-muted)] px-3 py-1.5">Repas : {kinds.meals}</span><span className="rounded-full bg-[var(--surface-muted)] px-3 py-1.5">Boissons : {kinds.beverages}</span><span className="ml-auto text-base font-black text-[var(--primary)]">{money(order.total_amount)}</span></div><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => setExpandedOrderId(expanded ? null : order.id)} className="rounded-lg border border-[var(--line)] px-3 py-2 text-xs font-black text-[var(--primary)]">{expanded ? "Masquer le détail" : "Voir le détail"}</button>{(order.status === "PENDING" || order.status === "IN_PREPARATION") && <button type="button" disabled={busyOrderId === order.id} onClick={() => void prepareOrder(order)} className="rounded-lg bg-[var(--primary)] px-3 py-2 text-xs font-black text-white">{busyOrderId === order.id ? "Préparation…" : "Préparer"}</button>}</div>{expanded && <div className="mt-4 rounded-xl bg-[var(--background)] p-4"><div className="mb-3 flex justify-between gap-3 text-xs font-bold text-[var(--muted)]"><span>{order.server_name || "Serveuse non renseignée"}</span><span>{order.table_label || "Comptoir"}</span></div>{(order.order_items ?? []).length ? <div className="space-y-2">{order.order_items?.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 border-b border-[var(--line)] pb-2 text-sm last:border-0 last:pb-0"><span><strong>{item.quantity} ×</strong> {item.product_name}<span className="ml-2 text-xs text-[var(--muted)]">{item.fulfillment_unit === "MEAL" ? "Repas" : "Boisson"}</span></span><span className="font-black">{money(item.total_price)}</span></div>)}<div className="flex justify-between gap-3 pt-2 text-sm font-black text-[var(--primary)]"><span>Total</span><span>{money(order.total_amount)}</span></div></div> : <p className="text-sm text-[var(--muted)]">Le détail des articles n’est pas disponible pour cette commande.</p>}</div>}</article>; })}</div>{!orders.length && <p className="py-10 text-center text-sm text-[var(--muted)]">Aucune commande ne correspond à ces filtres.</p>}</section>
  </div>;
}
