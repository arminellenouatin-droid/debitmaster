...
const statusLabel: Record<string, string> = { PENDING: "En attente", IN_PREPARATION: "En préparation", READY: "Prête à récupérer", HANDED_OFF: "Reçue par le serveur", DELIVERED: "Livrée au client", PAID: "Payée" };
...
export function ServeurClient({ tenantId, firstName, companyName, initialTab = "dashboard" }: { tenantId: string; firstName: string; companyName: string; initialTab?: "dashboard" | "orders" | "sales" | "profile" }) {
  ...
  const moveOrder = async (order: Order, status: "HANDED_OFF" | "DELIVERED") => {
    setBusy(true); setNotice("");
    ...
    if (!response.ok) return setNotice(result.error ?? "Impossible de mettre à jour la commande.");
    setNotice(status === "HANDED_OFF" ? "Réception enregistrée. Les articles sont maintenant affectés à votre service." : "Commande livrée. Vous pouvez enregistrer le paiement du client.");
    await refresh();
  };
  ...
  return <div className="space-y-7">
    ...
    {tab === "sales" && <section className="rounded-2xl bg-[var(--surface)] p-6"><p className="text-xs font-black uppercase tracking-[0.14em] text-[var(--secondary)]">Suivi personnel</p><h2 className="mt-2 text-xl font-black text-[var(--primary)]">Mes commandes</h2><div className="mt-5 space-y-3">{data.orders.map((order) => <div key={order.id} className="rounded-xl border border-[var(--line)] p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p className="font-black text-[var(--primary)]">{order.order_number} · {order.table_label || "Comptoir"}</p><p className="mt-1 text-xs text-[var(--muted)]">{new Date(order.created_at).toLocaleString("fr-FR")} · {statusLabel[order.status] || order.status}</p></div><p className="font-black text-[var(--primary)]">{money(order.total_amount)}</p></div>{order.status === "READY" && <button disabled={busy} onClick={() => void moveOrder(order, "HANDED_OFF")} className="mt-4 rounded-lg bg-[var(--primary)] px-3 py-2 text-xs font-black text-white">Confirmer « Reçu »</button>}{order.status === "HANDED_OFF" && <button disabled={busy} onClick={() => void moveOrder(order, "DELIVERED")} className="mt-4 rounded-lg bg-[var(--primary)] px-3 py-2 text-xs font-black text-white">Confirmer « Livré au client »</button>}{order.status === "DELIVERED" ? <div className="mt-4 flex flex-wrap gap-2"><button disabled={busy} onClick={() => void pay(order, "cash")} className="rounded-lg bg-[var(--primary)] px-3 py-2 text-xs font-black text-white">Encaisser cash</button><button disabled={busy} onClick={() => void pay(order, "moneroo")} className="rounded-lg border border-[var(--primary)] px-3 py-2 text-xs font-black text-[var(--primary)]">Mobile Money · Moneroo</button></div> : order.status === "PAID" ? <p className="mt-3 text-xs font-bold text-[#17623c]">Paiement enregistré et affectation soldée.</p> : order.status !== "READY" && order.status !== "HANDED_OFF" ? <p className="mt-3 text-xs font-bold text-[var(--muted)]">La commande sera disponible après préparation par le Gérant.</p> : null}</div>)}{!data.orders.length && <p className="py-8 text-sm text-[var(--muted)]">Aucune commande personnelle enregistrée.</p>}</div></section>}
