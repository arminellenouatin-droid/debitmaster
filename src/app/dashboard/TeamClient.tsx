...
export function TeamClient() {
  ...
  const [invites, setInvites] = useState<{ id: string; email: string; first_name: string; last_name: string; position: string; status: string }[]>([]);
  ...
  async function invite(event: FormEvent) {
    ...
    const response = await fetch("/api/invitations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tenantId, email: inviteEmail, firstName, lastName, position }) });
    const data = await response.json();
    if (!response.ok) return setError(data.error ?? "Impossible d’enregistrer l’invitation.");
    setInvites((current) => [data.invitation, ...current]); setInviteEmail("");
  }
