...
export function SubscriptionPlans({ tenantId }: { tenantId: string }) {
  ...
  return <section id="plans" className="rounded-xl border border-[var(--line)] bg-[var(--surface)] p-6 lg:col-span-2">
    ...
    <div className="mt-4 grid gap-4 rounded-xl border border-[var(--line)] bg-[var(--background)] p-5 md:grid-cols-2"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-[var(--secondary)]">Contenu de l’offre {activeActivity.label}</p><ul className="mt-3 space-y-2 text-sm leading-5 text-[var(--muted)]">{activeActivity.includedServices.map((service) => <li key={service} className="flex gap-2"><span className="font-black text-[var(--secondary)]">✓</span><span>{service}</span></li>)}</ul></div><div><p className="text-xs font-black uppercase tracking-[0.14em] text-[var(--secondary)]">Services communs inclus</p><ul className="mt-3 space-y-2 text-sm leading-5 text-[var(--muted)]">{activeActivity.commonServices.map((service) => <li key={service} className="flex gap-2"><span className="font-black text-[var(--secondary)]">✓</span><span>{service}</span></li>)}</ul></div></div>
    ...
    <div className="mt-6 flex flex-col justify-between gap-4 rounded-xl bg-[var(--primary)] p-5 text-white sm:flex-row sm:items-center"><div><p className="text-sm font-black">{selected ? `Formule sélectionnée : ${selected.label}` : "Sélectionnez une formule"}</p><p className="mt-1 text-xs leading-5 text-white/65">Montant de cette formule : {selected ? `${money.format(selected.priceXof)} XOF au total` : "sélectionnez une formule"}. Le règlement utilise le tarif du type enregistré : {actualActivity?.label ?? data.activity.type}.</p></div><button type="button" disabled={!selected || pending} onClick={() => void startPayment()} className="rounded-lg bg-[var(--secondary-container)] px-5 py-3 text-sm font-black text-[var(--primary)] disabled:cursor-not-allowed disabled:opacity-50">{pending ? "Redirection vers Moneroo…" : selected ? `Payer ${money.format(paymentPrice)} XOF` : "Choisir une formule"}</button></div>
