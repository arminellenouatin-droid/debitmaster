...
export default async function StockPage() {
  ...
  if ((context.role === "ADMINISTRATEUR" || context.role === "SUPERVISEUR") && (active.company?.activity_type === "HOTEL_AUBERGE" || active.company?.activity_type === "POWER")) return <DashboardShell firstName={firstName}><OwnerStockClient tenantId={active.tenantId ?? ""} companyName={active.company?.name ?? "Établissement actif"} role={context.role === "SUPERVISEUR" ? "SUPERVISEUR" : "ADMINISTRATEUR"} /></DashboardShell>;
