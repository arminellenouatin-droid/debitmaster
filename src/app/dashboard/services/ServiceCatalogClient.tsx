...
type Service = { id: string; name: string; description: string | null; price_xof: number; billing_unit: string; image_url: string | null; is_active: boolean };
...
export function ServiceCatalogClient({ tenantId, activityCode }: { tenantId: string; activityCode: ActivityCode }) {
  ...
  const [name, setName] = useState(""); const [description, setDescription] = useState(""); const [price, setPrice] = useState(""); const [unit, setUnit] = useState("UNIT"); const [imageUrl, setImageUrl] = useState("");
  ...
  function reset() { setEditingId(null); setName(""); setDescription(""); setPrice(""); setUnit("UNIT"); setImageUrl(""); }
  function edit(service: Service) { setEditingId(service.id); setName(service.name); setDescription(service.description ?? ""); setPrice(String(service.price_xof)); setUnit(service.billing_unit); setImageUrl(service.image_url ?? ""); setError(""); setNotice(""); }
  async function submit(event: FormEvent) {
    event.preventDefault(); setBusy(true); setError(""); setNotice("");
    try {
      const response = await fetch("/api/power/services", { method: editingId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tenantId, activityCode, serviceId: editingId ?? undefined, name, description, priceXof: Number(price), billingUnit: unit, imageUrl, isActive: true }) });
