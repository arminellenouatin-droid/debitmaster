...
type Company = { id: string; name: string; activity_type: string; country: string; currency: string; status: string; zones_tables_enabled?: boolean; trial_ends_at?: string | null; subscription_plan?: string | null; subscription_expires_at?: string | null };
type Overview = {
  company: Company;
  role: string;
  isOwner: boolean;
...
export function DashboardClient() {
  ...
  const [savingOrderRule, setSavingOrderRule] = useState(false);
  ...
  async function updateOrderRule(enabled: boolean) {
    setSavingOrderRule(true); setError("");
    try {
      const response = await fetch("/api/companies/order-settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tenantId: selectedTenantId, zonesTablesEnabled: enabled }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error ?? "Impossible d’enregistrer ce réglage.");
      setOverview((current) => current ? { ...current, company: { ...current.company, zones_tables_enabled: result.zonesTablesEnabled } } : current);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Impossible d’enregistrer ce réglage."); } finally { setSavingOrderRule(false); }
  }
  ...
  return <div className="space-y-7">
    ...
    {overview.isOwner && <section className="flex flex-col justify-between gap-4 rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5 sm:flex-row sm:items-center"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--secondary)]">Règle de prise de commande</p><h3 className="mt-2 text-xl font-black text-[var(--primary)]">Exiger une zone et une table</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">Lorsque cette option est active, les serveuses doivent renseigner la zone et le numéro de table avant d’envoyer une commande.</p></div><label className="flex shrink-0 cursor-pointer items-center gap-3 text-sm font-black text-[var(--primary)]"><input type="checkbox" checked={overview.company.zones_tables_enabled !== false} onChange={(event) => void updateOrderRule(event.target.checked)} disabled={savingOrderRule} className="h-5 w-5 accent-[var(--primary)]" />{overview.company.zones_tables_enabled !== false ? "Activée" : "Désactivée"}</label></section>}
