...
type MenuData = { company: { id: string; name: string; phone?: string | null; address?: string | null }; table: { id: string; label: string; zone: string | null } | null; target: { id: string; label: string; zone: string; kind: "TABLE" | "ROOM" }; products: Product[]; categories: Category[]; activities?: Activity[]; services?: Service[]; rooms?: Room[]; wifiTickets?: WifiTicket[] };
type ServiceActivity = "GYM" | "LAVAGE" | "LODGING" | "WIFI";
type CartLine =
  | { kind: "product"; product: Product; quantity: number }
  | { kind: "service"; activityCode: ServiceActivity; serviceId?: string; roomId?: string; label: string; unitPrice: number; quantity: number; detail?: string };
...
const lineKey = (line: CartLine) => line.kind === "product" ? `product:${line.product.id}` : `service:${line.activityCode}:${line.serviceId ?? line.roomId ?? line.label}`;
...
export function MenuClient({ token }: { token: string }) {
  ...
  const total = cart.reduce((sum, line) => sum + (line.kind === "product" ? line.product.price : line.unitPrice) * line.quantity, 0);
  const itemCount = cart.reduce((sum, line) => sum + line.quantity, 0);
  const rooms = data?.rooms ?? [];
  const services = data?.services ?? [];
  const targetLabel = data?.target.label ?? data?.table?.label ?? "Votre emplacement";
  const targetZone = data?.target.zone ?? data?.table?.zone ?? "—";

  function add(product: Product) { setSuccess(""); setCart((current) => { const found = current.find((line) => line.kind === "product" && line.product.id === product.id); return found ? current.map((line) => line.kind === "product" && line.product.id === product.id ? { ...line, quantity: line.quantity + 1 } : line) : [...current, { kind: "product", product, quantity: 1 }]; }); }
  function addService(line: Omit<Extract<CartLine, { kind: "service" }>, "kind">) { setSuccess(""); setCart((current) => { const next = { kind: "service", ...line } as Extract<CartLine, { kind: "service" }>; const found = current.find((entry) => lineKey(entry) === lineKey(next)); return found ? current.map((entry) => lineKey(entry) === lineKey(next) ? { ...entry, quantity: entry.quantity + 1 } : entry) : [...current, next]; }); }
  function addRoom(room: Room) { if (!isAvailable(room)) { setError("Cette chambre est actuellement occupée."); return; } addService({ activityCode: "LODGING", roomId: room.id, label: `Chambre ${room.room_number} · Pass`, unitPrice: room.pass_price_xof, quantity: 1, detail: `${room.pass_duration_minutes} minutes` }); }
  function change(key: string, delta: number) { setCart((current) => current.flatMap((line) => lineKey(line) !== key ? [line] : line.quantity + delta > 0 ? [{ ...line, quantity: line.quantity + delta }] : [])); }
  async function submitOrder() {
    if (!cart.length) return;
    setSubmitting(true); setError(""); setSuccess("");
    try {
      const response = await fetch(`/api/public/menu/${encodeURIComponent(token)}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        lines: cart.filter((line): line is Extract<CartLine, { kind: "product" }> => line.kind === "product").map((line) => ({ productId: line.product.id, quantity: line.quantity })),
        serviceLines: cart.filter((line): line is Extract<CartLine, { kind: "service" }> => line.kind === "service").map((line) => ({ activityCode: line.activityCode, serviceId: line.serviceId, roomId: line.roomId, label: line.label, unitPrice: line.unitPrice, quantity: line.quantity, detail: line.detail })),
        customerName, note
      }) });
  ...
  return <main className="qr-page">
    ...
    <section className="qr-hero"><div className="qr-hero-glow" aria-hidden="true" /><div className="qr-hero-art" aria-hidden="true" /><div className="qr-hero-copy"><div className="qr-kicker-row"><span className="qr-hero-symbol">◒</span><p className="qr-kicker">{data.target.kind === "ROOM" ? "CHAMBRE" : "TABLE"} {targetLabel}{targetZone ? ` · ${targetZone}` : ""}</p></div><h1>Choisissez votre moment.</h1><p>Une carte pensée pour les bons moments : commandez, nous préparons et nous vous servons à table.</p><button className="qr-button qr-button-light" onClick={() => document.getElementById("repas")?.scrollIntoView({ behavior: "smooth" })}>Explorer la carte <span aria-hidden="true">→</span></button></div></section>
    ...
    <section id="auberge" className="qr-section qr-stay"><div className="qr-section-heading"><div><p className="qr-kicker">Un peu plus longtemps</p><h2>Auberge</h2></div></div><div className="qr-stay-panel"><div className="qr-stay-art" aria-hidden="true"><span>⌂</span></div><div className="qr-stay-overlay"><span className={`qr-availability ${rooms.some(isAvailable) ? "available" : "busy"}`}><i />{rooms.some(isAvailable) ? "Chambres disponibles" : "Toutes occupées"}</span><h3>Un espace pour souffler.</h3><p>{rooms.length ? `${rooms.filter(isAvailable).length} chambre(s) actuellement disponible(s).` : "Découvrez nos chambres et nos options de passage."}</p><div className="qr-room-options">{rooms.filter(isAvailable).slice(0, 4).map((room) => <button className="qr-room-option" key={room.id} onClick={() => addRoom(room)}><span>Chambre {room.room_number}</span><strong>{money(room.pass_price_xof)}</strong></button>)}</div></div></div></section>
    {(services.length || (data.wifiTickets ?? []).length) ? <section className="qr-section qr-services"><div className="qr-section-heading"><div><p className="qr-kicker">Les services de la maison</p><h2>Plus qu’un menu</h2></div></div><div className="qr-service-grid">{services.slice(0, 4).map((service) => <div className="qr-service-card" key={service.id}><span className="qr-service-icon">✦</span><div><h3>{service.name}</h3><p>{service.description || service.billing_unit || "Sur demande"}</p><strong>{money(service.price_xof)}</strong></div><button className="qr-add qr-add-outline" onClick={() => addService({ activityCode: ((data.activities ?? []).find((activity) => activity.id === service.activity_id)?.activity_code as ServiceActivity | undefined) ?? "GYM", serviceId: service.id, label: service.name, unitPrice: service.price_xof, quantity: 1, detail: service.billing_unit ?? undefined })} aria-label={`Ajouter ${service.name}`}>+</button></div>)}{(data.wifiTickets ?? []).map((ticket) => <div className="qr-service-card" key={ticket.ticket_code}><span className="qr-service-icon">⌁</span><div><h3>{ticket.label}</h3><p>{ticket.duration_label}</p><strong>{money(ticket.unit_price_xof)}</strong></div><button className="qr-add qr-add-outline" onClick={() => addService({ activityCode: "WIFI", label: ticket.label, unitPrice: ticket.unit_price_xof, quantity: 1, detail: ticket.ticket_code })} aria-label={`Ajouter ${ticket.label}`}>+</button></div>)}</div></section> : null}
    <footer className="qr-footer"><span>{data.company.name}</span><span>{data.target.kind === "ROOM" ? "Chambre" : "Table"} {targetLabel}</span><span>Commande au service</span></footer>
