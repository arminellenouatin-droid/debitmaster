...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await context.supabase.from("inventory_audits").select("id,tenant_id,title,scope,inventory_type,status,counted_at,submitted_at,validated_at,validated_by,closed_at,created_by,note,created_at,updated_at,inventory_audit_items(id,product_id,theoretical_quantity,physical_quantity,unit_cost,variance_quantity,variance_value,cause,justification,counted_by,counted_at,products(id,name,unit,stock_family))").eq("tenant_id", tenantId).order("created_at", { ascending: false }).limit(50);
...
export async function POST(request: NextRequest) {
  ...
  if (action === "CREATE") {
    ...
    const { data: audit, error } = await context.supabase.from("inventory_audits").insert({ tenant_id: tenantId, title: body.title?.trim() || `Inventaire du ${new Intl.DateTimeFormat("fr-FR").format(new Date())}`, scope: body.scope || "ALL_STOCK", inventory_type: body.inventoryType || "FULL", created_by: context.user.id, note: body.note?.trim() || null }).select("id,title,status,counted_at").single();
    if (error) return NextResponse.json({ error: "Impossible de créer la session d’inventaire." }, { status: 500 });
    return NextResponse.json({ audit }, { status: 201 });
  }
  if (!body.auditId) return NextResponse.json({ error: "Session d’inventaire manquante." }, { status: 400 });
  const { data: audit, error: auditError } = await context.supabase.from("inventory_audits").select("id,status,tenant_id").eq("id", body.auditId).eq("tenant_id", tenantId).maybeSingle();
  ...
  if (action === "SUBMIT") {
    ...
    const { data: items } = await context.supabase.from("inventory_audit_items").select("physical_quantity,theoretical_quantity,justification,unit_cost").eq("audit_id", audit.id).limit(5000);
    ...
    const { error } = await context.supabase.from("inventory_audits").update({ status: "SUBMITTED", submitted_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", audit.id).eq("status", "DRAFT");
  ...
  if (action === "APPROVE" || action === "CLOSE") {
    ...
    const { error } = await context.supabase.from("inventory_audits").update(patch).eq("id", audit.id);
