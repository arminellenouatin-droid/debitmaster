...
export async function GET(request: Request) {
  try {
    ...
    const range = new URL(request.url).searchParams.get("range") === "7d" ? 7 : 30;
    ...
    const [employeesResult, ordersResult, paymentsResult, productsResult, tablesResult, commissionsResult] = await Promise.all([
      ...
      context.supabase.from("orders").select("id,order_number,server_user_id,server_name,table_label,status,total_amount,currency,received_by_user_id,received_at,delivered_by_user_id,delivered_at,created_at,updated_at").eq("tenant_id", tenantId).gte("created_at", since).order("created_at", { ascending: false }).limit(500),
