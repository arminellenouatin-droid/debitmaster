/* DebitManager Power owner sales API: paid revenue only, isolated by active tenant. */
...
type Order = { id: string; total_amount: number | null; server_name: string | null; server_user_id: string | null; created_at: string; order_items?: { quantity: number; total_price: number | null; fulfillment_unit: string | null }[] };
...
export async function GET(request: Request) {
  try {
    ...
    const [ordersResult, paymentsResult, employeesResult, activitiesResult] = await Promise.all([
      context.supabase.from("orders").select("id,total_amount,server_name,server_user_id,created_at,order_items(quantity,total_price,fulfillment_unit)").eq("tenant_id", tenantId).gte("created_at", since).lte("created_at", until).order("created_at", { ascending: false }).limit(2000),
      context.supabase.from("payments").select("order_id,amount,status,created_at").eq("tenant_id", tenantId).gte("created_at", since).lte("created_at", until).limit(4000),
      context.supabase.from("employees").select("user_id,first_name,last_name,position,status").eq("tenant_id", tenantId).is("deleted_at", null).limit(500),
      context.supabase.from("company_activities").select("id,name,is_active").eq("tenant_id", tenantId).order("name").limit(50),
    ]);
    if (ordersResult.error || paymentsResult.error || employeesResult.error || activitiesResult.error) return NextResponse.json({ error: "Impossible de charger le chiffre d’affaires." }, { status: 500 });
    ...
    for (const order of paidOrders) {
      ...
      for (const item of order.order_items ?? []) { const activity = activityMap.get(itemActivity(item.fulfillment_unit))!; activity.amount += Number(item.total_price ?? 0); activity.units += Number(item.quantity ?? 0); activity.orderCount += 1; }
    }
    ...
    const configuredActivities = (activitiesResult.data ?? []).map((activity) => activity.name);
    return NextResponse.json({ period: { range: url.searchParams.get("range") ?? "30d", start: since.slice(0, 10), end: until.slice(0, 10) }, metrics: { totalRevenue, paidOrderCount: paidOrders.length, averageBasket: paidOrders.length ? Math.round(totalRevenue / paidOrders.length) : 0 }, activities: Array.from(activityMap.values()).map((item) => ({ ...item, note: item.amount === 0 && !["Boissons", "Repas"].includes(item.name) ? "Aucune vente enregistrée sur cette période" : undefined })), configuredActivities, people: Array.from(personMap.values()).sort((a, b) => b.amount - a.amount), peopleOptions: (employeesResult.data ?? []).map((employee) => ({ id: employee.user_id, name: `${employee.first_name ?? ""} ${employee.last_name ?? ""}`.trim() || "Sans nom", position: employee.position })), daily });
  } catch { return NextResponse.json({ error: "Service de chiffre d’affaires temporairement indisponible." }, { status: 500 }); }
}
