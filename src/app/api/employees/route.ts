...
export async function GET(request: Request) {
  try {
    ...
    const query = context.supabase.from("employees").select("id,tenant_id,user_id,first_name,last_name,phone,position,status,must_change_password,approved_at,created_at").is("deleted_at", null).order("created_at", { ascending: false }).limit(100);
