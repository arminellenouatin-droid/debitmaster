...
export async function POST(request: Request) {
  try {
    ...
    const { data: order, error: orderError } = await supabase.from("orders").select("id,tenant_id,order_number,total_amount,currency,status").eq("id", orderId).eq("tenant_id", tenantId).maybeSingle();
