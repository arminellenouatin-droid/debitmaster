...
export async function POST(request: Request) {
  ...
  const affiliate = await admin.from("platform_affiliates").insert({ user_id: userId, code, display_name: displayName, commission_rate: 20, payout_threshold_xof: 20000, status: "ACTIVE" }).select("id,user_id,code,display_name,commission_rate,payout_threshold_xof,status,created_at").single();
  if (affiliate.error) {
