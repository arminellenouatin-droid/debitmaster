...
import { billingPeriodCodes, subscriptionActivityCodes, subscriptionPlanCodes, getSubscriptionActivityCatalog, normalizeActivityCode } from "@/lib/subscription-plans";
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!context.user) return errorResponse("Authentification requise.", 401);
    if (!context.isPlatformAdmin) return errorResponse("Accès super-administration requis.");
    const body = await request.json() as { activityCode?: string; planCode?: string; billingPeriod?: string; priceXof?: number; description?: string };
    ...
    if (!subscriptionActivityCodes.includes(activityCode as (typeof subscriptionActivityCodes)[number]) || !subscriptionPlanCodes.includes(planCode as (typeof subscriptionPlanCodes)[number]) || !billingPeriodCodes.includes(billingPeriod as (typeof billingPeriodCodes)[number]) || !Number.isSafeInteger(priceXof) || priceXof <= 0) {
      return errorResponse("Type d’établissement, période et prix valides requis.", 400);
    }

    const { data, error } = await context.supabase
      .from("saas_plan_prices")
      .upsert({ activity_code: activityCode, plan_code: planCode, billing_period: billingPeriod, price_xof: priceXof, description, is_active: true, updated_by: context.user.id, updated_at: new Date().toISOString() }, { onConflict: "activity_code,plan_code,billing_period" })
      .select("id,activity_code,plan_code,billing_period,price_xof,description,is_active,updated_at")
      .single();
