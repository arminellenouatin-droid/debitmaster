...
export async function POST(request: Request) {
  try {
    ...
    await emitTenantNotification({ tenantId, actorUserId: context.user!.id, subject: `Préparation affectée · ${item.product_name}`, body: `${quantity} unité(s) de ${item.product_name} sont affectées à ${cook.first_name} ${cook.last_name}.`, eventType: "MEAL_ASSIGNMENT_CREATED", entityId: orderId, actionPath: `/dashboard/kitchen?orderId=${encodeURIComponent(orderId)}`, actionPermission: "orders.prepare", operatorPositions: ["CUISINIER"], dedupeKey: `meal-assignment:${assignment.id}`, metadata: { assignmentId: assignment.id, orderItemId, cookEmployeeId, quantity } });
