// DebitManager plan spécial settings: owner-only preferences and encrypted MTN MoMo credentials, never returned in plaintext.
...
export async function GET(request: Request) {
  try {
    ...
    if (!(await isSpecialOwner(context, tenantId))) return NextResponse.json({ error: "Seul le propriétaire d’un établissement plan spécial peut consulter ces réglages." }, { status: 403 });
    ...
    if (companyError || credentialsError) return NextResponse.json({ error: "Impossible de charger les réglages plan spécial." }, { status: 500 });
    ...
    return NextResponse.json({ error: "Réglages plan spécial temporairement indisponibles." }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    const context = await getAuthorizationContext();
    if (!(await isSpecialOwner(context, tenantId))) return NextResponse.json({ error: "Seul le propriétaire d’un établissement plan spécial peut modifier ces réglages." }, { status: 403 });
    ...
    if (Object.keys(updates).length) {
      ...
      if (error) return NextResponse.json({ error: "Impossible d’enregistrer les réglages plan spécial." }, { status: 400 });
    }
    if (body.credentials) {
    ...
    return NextResponse.json({ ok: true, message: "Réglages plan spécial enregistrés. Les secrets complets ne sont jamais renvoyés." });
  } catch (cause) {
    const message = cause instanceof Error ? cause.message : "Réglages plan spécial temporairement indisponibles.";
    return NextResponse.json({ error: message.includes("ENCRYPTION_KEY") ? "Le chiffrement serveur n’est pas encore configuré." : "Réglages plan spécial temporairement indisponibles." }, { status: 500 });
  }
}
