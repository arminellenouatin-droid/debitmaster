...
export async function POST(request: Request) {
  try {
    ...
    const admin = createSupabaseAdminClient(); const { data, error } = await admin.from("power_lodging_rooms").insert({ tenant_id: tenantId, room_number: roomNumber, pass_price_xof: passPriceXof, pass_duration_minutes: passDurationMinutes, night_price_xof: nightPriceXof, night_duration_nights: nightDurationNights }).select("id,tenant_id,room_number,pass_price_xof,pass_duration_minutes,night_price_xof,night_duration_nights,is_active,occupied_started_at,occupied_until").single();
...
export async function PATCH(request: Request) {
  try {
    ...
    const admin = createSupabaseAdminClient(); const { data, error } = await admin.from("power_lodging_rooms").update({ ...patch, updated_at: new Date().toISOString() }).eq("id", roomId).eq("tenant_id", tenantId).select("id,tenant_id,room_number,pass_price_xof,pass_duration_minutes,night_price_xof,night_duration_nights,is_active,occupied_started_at,occupied_until").single();
