// DebitManager plan spécial: activités multi-départements, accessibles uniquement dans un établissement sous plan POWER.
...
async function specialTenant(context: Awaited<ReturnType<typeof getAuthorizationContext>>, tenantId: string) {
  ...
  const { data } = await context.supabase.from("companies").select("id,activity_type").eq("id", tenantId).eq("subscription_plan", "SPECIAL").is("deleted_at", null).maybeSingle();
  return Boolean(data);
}
...
export async function GET(request: Request) {
  ...
  if (!await specialTenant(context, tenantId) || !can(context, "activities.view")) return NextResponse.json({ error: "Accès aux activités plan spécial refusé." }, { status: 403 });
  ...
  if (error) return NextResponse.json({ error: "Impossible de charger les activités plan spécial." }, { status: 500 });
...
export async function POST(request: Request) {
  try {
    ...
    const context = await getAuthorizationContext();
    if (!context.user) return NextResponse.json({ error: "Authentification requise." }, { status: 401 });
    if (!await specialTenant(context, tenantId) || !can(context, "activities.manage")) return NextResponse.json({ error: "Permission insuffisante pour gérer les activités plan spécial." }, { status: 403 });
    ...
    if (error) return NextResponse.json({ error: "Impossible de créer l’activité plan spécial." }, { status: 400 });
...
export async function PATCH(request: Request) {
  try {
    ...
    if (error || !data) return NextResponse.json({ error: "Impossible de mettre à jour l’activité plan spécial." }, { status: 400 });
