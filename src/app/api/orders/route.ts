...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase.from("orders").select("id,tenant_id,order_number,table_label,customer_id,server_user_id,server_name,received_by_user_id,received_at,delivered_by_user_id,delivered_at,status,total_amount,currency,created_at,order_items(id,product_id,product_name,quantity,unit_price,total_price),order_stock_allocations(id,server_user_id,product_id,quantity,status,allocated_at,settled_at)").order("created_at", { ascending: false }).limit(50);
