...
const navigation: ReadonlyArray<readonly [string, string, string]> = [
  ["⌂", "Dashboard", "/dashboard"], ["🍽", "Repas / Cuisine", "/dashboard/meals"], ["▦", "Plan de salle", "/dashboard/tables"], ["＋", "Ventes", "/dashboard/orders"], ["◉", "Commandes", "/dashboard/orders"], ["▰", "Vente", "/dashboard/sales"], ["▤", "Gestion des stocks", "/dashboard/stock"], ["⇩", "Approvisionnement", "/dashboard/supply"], ["◌", "WIFI", "/dashboard/wifi"], ["♙", "Personnel", "/dashboard/personnel"], ["◫", "Finance", "/dashboard/finance"], ["✉", "Messages", "/dashboard/messages"], ["✦", "Gestion Power", "/dashboard/power"], ["⚙", "Profil", "/dashboard/settings"],
];
...
export async function DashboardShell({ children, firstName }: { children: React.ReactNode; firstName: string }) {
  ...
  const baseNavigation: ReadonlyArray<NavItem> = serviceRole ? serviceNavigation[serviceRole] : isPowerSupervisor ? navigation.filter(([, label]) => label !== "Ventes") : activeContext.role === "SERVEUR" ? navigation.filter(([, label]) => ["Dashboard", "Commandes", "Profil"].includes(label)) : activeContext.role === "CHEF_CUISINE" || activeContext.role === "CUISINIER" ? navigation.filter(([, label]) => ["Dashboard", "Repas / Cuisine", "Profil"].includes(label)) : activeContext.role === "MAGASINIER" || activeContext.role === "INVENTAIRE" || activeContext.role === "RESPONSABLE_INVENTAIRE" ? navigation.filter(([, label]) => ["Dashboard", "Gestion des stocks", "Profil"].includes(label)) : activeContext.role === "GERANT" ? navigation.filter(([, label]) => ["Dashboard", "Commandes", "Profil"].includes(label) || (label === "Plan de salle" && activeContext.permissions.has("tables.view")) || (label === "Personnel" && activeContext.permissions.has("team.view")) || (label === "Finance" && activeContext.permissions.has("finance.view")) || (label === "WIFI" && activeContext.permissions.has("services.view"))) : navigation.filter(([, label]) => label !== "Ventes" && label !== "Approvisionnement");
