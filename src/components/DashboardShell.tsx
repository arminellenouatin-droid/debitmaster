...
const navigation: ReadonlyArray<readonly [string, string, string]> = [
  ["⌂", "Dashboard", "/dashboard"], ["📦", "Produits et services", "/dashboard/catalog"], ["🍽", "Repas / Cuisine", "/dashboard/meals"], ["▦", "Plan de salle", "/dashboard/tables"], ["＋", "Ventes", "/dashboard/orders"], ["◉", "Commandes", "/dashboard/orders"], ["▰", "Vente", "/dashboard/sales"], ["▤", "Gestion des stocks", "/dashboard/stock"], ["⇩", "Approvisionnement", "/dashboard/supply"], ["◌", "WIFI", "/dashboard/wifi"], ["♙", "Personnel", "/dashboard/personnel"], ["◫", "Finance", "/dashboard/finance"], ["✉", "Messages", "/dashboard/messages"], ["✦", "Gestion Power", "/dashboard/power"], ["⚙", "Profil", "/dashboard/settings"],
];
type NavItem = readonly [string, string, string];
const serviceNavigation: Record<"GYM" | "LAVAGE" | "AUBERGE", ReadonlyArray<NavItem>> = {
  GYM: [["⌂", "Dashboard", "/dashboard"], ["📦", "Produits et services", "/dashboard/catalog"], ["▰", "Vente", "/dashboard/service-sales"], ["▤", "Liste des services", "/dashboard/services?activity=GYM"], ["◫", "Abonnements", "/dashboard/subscriptions"], ["⚙", "Profil", "/dashboard/settings"]],
  LAVAGE: [["⌂", "Dashboard", "/dashboard"], ["📦", "Produits et services", "/dashboard/catalog"], ["▰", "Vente", "/dashboard/service-sales"], ["▤", "Liste des prestations", "/dashboard/services?activity=LAVAGE"], ["◫", "Caisse lavage", "/dashboard/cash"], ["⚙", "Profil", "/dashboard/settings"]],
  AUBERGE: [["⌂", "Dashboard", "/dashboard"], ["📦", "Produits et services", "/dashboard/catalog"], ["▰", "Vente", "/dashboard/service-sales"], ["◫", "Liste et occupation", "/dashboard/occupancy"], ["▤", "Caisse auberge", "/dashboard/cash"], ["⚙", "Profil", "/dashboard/settings"]],
} as const;
...
export async function DashboardShell({ children, firstName }: { children: React.ReactNode; firstName: string }) {
  ...
  const canSeeCatalog = isOwner || isPowerSupervisor || activeContext.role === "GERANT" || activeContext.role === "GERANT_ADJOINT" || activeContext.role === "MAGASINIER" || activeContext.role === "CHEF_CUISINE" || activeContext.permissions.has("products.manage") || activeContext.permissions.has("services.manage");
  const baseNavigation: ReadonlyArray<NavItem> = serviceRole ? serviceNavigation[serviceRole] : isPowerSupervisor ? navigation.filter(([, label]) => label !== "Ventes") : activeContext.role === "SERVEUR" ? navigation.filter(([, label]) => ["Dashboard", "Commandes", "Profil"].includes(label)) : activeContext.role === "CHEF_CUISINE" || activeContext.role === "CUISINIER" ? navigation.filter(([, label]) => ["Dashboard", "Produits et services", "Repas / Cuisine", "Profil"].includes(label)) : activeContext.role === "MAGASINIER" ? navigation.filter(([, label]) => ["Dashboard", "Produits et services", "Gestion des stocks", "Profil"].includes(label)) : activeContext.role === "GERANT" || activeContext.role === "GERANT_ADJOINT" ? navigation.filter(([, label]) => ["Dashboard", "Produits et services", "Commandes", "Profil"].includes(label) || (label === "Plan de salle" && activeContext.permissions.has("tables.view")) || (label === "Personnel" && activeContext.permissions.has("team.view")) || (label === "Finance" && activeContext.permissions.has("finance.view")) || (label === "WIFI" && activeContext.permissions.has("services.view"))) : navigation.filter(([, label]) => label !== "Ventes" && label !== "Approvisionnement");
  ...
  return <div className="min-h-screen bg-[var(--background)] lg:pl-64">
    ...
    <DashboardHeader firstName={firstName} companyName={companyName} tenantId={activeContext.company?.id ?? ""} role={role} isOwner={isOwner} subscriptionStatus={subscriptionStatus} avatarUrl={avatarUrl} navigationItems={visibleNavigation} />
